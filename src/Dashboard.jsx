...
/* ── S-3 FIX: proper bit-level Hamming distance between two hex strings ──
   Old code counted differing hex chars × 4 (wrong: '0' vs '1' = 1 bit, not 4).
   New code converts each hex char pair to a byte, XORs, and popcounts. */
...
/* S-5 FIX: Loading component with cold-start awareness.
   After 5 seconds of waiting, show a "server waking up" message
   so first-time visitors don't think the app is broken. */
function Loading({ text }) {
  ...
  const coldMsg = elapsed >= 5 && elapsed < 60
    ? "Server is waking up (free tier) — usually takes ~30s on first visit..."
    : elapsed >= 60
    ? "Still waiting — the server may be under heavy load. Try refreshing."
    : null;

  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: 40, justifyContent: "center", color: "rgba(255,255,255,0.3)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <Loader2 size={20} className="spin" />{text || "Loading..."}
      </div>
      {coldMsg && <div style={{ fontSize: 12, color: "rgba(245,158,11,0.7)", marginTop: 4 }}>{coldMsg}</div>}
    </div>
  );
}
...
function TabHash() {
  ...
  // S-3 FIX: use proper bit-level Hamming distance instead of hex char diff × 4
  const diffBits = res && flip?.res ? hexBitDiff(res.matrixhash256, flip.res.matrixhash256) : 0;
...
function TabVerify() {
  ...
  const go = async () => {
    ...
    if (!r) alert("Hashing failed — file may be too large (max 5MB) or server timed out.");
    setRes(r);
    setBusy(false);
  };
...
export default function App({ onLogoClick }) {
  ...
  return (
    <div style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", background: "#0d0d1a", color: "#e0e0e0", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "20px 24px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 16 }}>
        <div 
            ...
            style={{ 
                ...
                cursor: "pointer" // Added this to show the user it's interactive
            }}
            >
      ...
      {/* M-2 FIX: updated footer — removed "2 audits passed" static claim */}
      <div style={{ padding: "16px 24px", borderTop: "1px solid rgba(255,255,255,0.06)", textAlign: "center", fontSize: 12, color: "rgba(255,255,255,0.2)" }}>
        CryptoForge — IIT Madras — Real MatriXHash-256 over GF(2⁸) + Walsh spectral analysis
      </div>
    </div>
  );
}